jQuery(function ($) {
    ...
    $(document).on('click', '[name="homerunner_search_droppoints"]', function () {
        var _ = $('.homerunner_select_shop'),
            ...
            chosen_zip_code = $('[name="homerunner_zip_code_search"]'),
        ...
        var ajax_data = {
            action: 'homerunner_droppoint_search',
        ...
        if (chosen_zip_code.length !== 0) {
            $('.homerunner-droppoints').slideUp(250, function () {
    ...
    $(document).on('click', '.shipping_method', function () {
        ...
        $('#homerunner_zip_code_search').attr('placeholder', origin_zip_code);
        $('#homerunner-search-results').hide();
    });

    $(document).on('keydown keypress', '[name="homerunner_zip_code_search"]', function (e) {
        if (e.keyCode === 13) {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();

            $('#homerunner_search_droppoints').click();
    ...
    (function checkout_digest() {
        var droppoint_container = $('.woocommerce-checkout-review-order .homerunner_select_shop');
            ...
            if ($('[name="shipping_method[0]"]').length) {
                ...
                if (val !== undefined) {
                    ...
                    if (droppoint_container.attr('data-carrier') !== carrier) {
                        ...
                        droppoint_container.find('.homerunner-droppoints').slideUp(250, function () {
                            $(this).html('');
                        }).slideDown(250);
                    }

                    if (val !== undefined && val.indexOf('homerunner') === 0) {

                        if (service !== null && service.indexOf('droppoint') === 0) {
                            droppoint_container.filter('.hidden').slideDown(250, function () {
                                $(this).removeClass('hidden');
                            });
                            $('[name="homerunner_carrier"]').val(carrier);
                        } else {
                            droppoint_container.filter(':not(.hidden)').addClass('hidden').stop().slideUp(250, function () {
                                droppoint_container.find('.homerunner-droppoints').html('');
                                droppoint_container.find('.homerunner-droppoints').find('input').remove();
                            });
                        }
                    } else {
                        droppoint_container.filter(':not(.hidden)').addClass('hidden').stop().slideUp(250, function () {
