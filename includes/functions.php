<?php
/**
 * Created by HomeRunner.
 * Developer: Kevin Hansen
 */

// Add HomeRunner to navigation
add_action( 'admin_menu', function () {
    add_menu_page( 'HomeRunner - SmartCheckout', 'HomeRunner - SmartCheckout', 'manage_options', 'csc-options', function () {
    ...
    }, plugins_url( 'assets/images/homerunner-logo.png', CSC_PLUGIN_FILE ) );
} );
    ...
    if ($shipping_method = CSC::getCSCShippingMethod($order->get_id())) {
        ...
        <div id="csc-shipment-information">
            ...
            <div class="csc-shipment-header">
                <div class="col1"><h3>HomeRunner</h3></div>
                <div class="col2">
                ...
                <?php if(isset(get_post_meta($order->get_id(), '_csc_shipments')[0])): ?>
                    <?php foreach (get_post_meta($order->get_id(), '_csc_shipments')[0] as $shipment): ?>
                        <?php if(get_option('csc_warehouse') == 'normal'): ?>
                            <div class="csc-shipment-line">
                                <div class="csc-shipment-image"><img class="printlabel" data-packagenumber="<?php echo $shipment['package_number']; ?>" src="/wp-content/plugins/woocommerce_smartcheckout/assets/images/pdf.png" height="35px"></div>
                                <div class="csc-shipment-data">
                                    ...
                                    <div><a target="_blank" href="https://tracking.homerunner.com/?shipment=<?php echo $shipment['package_number']; ?>"><?php echo __('Track this shipment', 'csc_textdomain'); ?></a></div>
                                </div>
                                <div class="csc-shipment-price">
                                    <div><b>Subtotal</b></div>
                        ...
                        <?php elseif (get_option('csc_warehouse') == 'pcn'): ?>
                            <div class="csc-shipment-line">
                                <div class="csc-shipment-image"><img src="/wp-content/plugins/homerunner_smartcheckout/assets/images/print.png" height="30px"></div>
                                <div class="csc-shipment-data">
                                    <div><b><?php echo $shipment['package_number']; ?></b></div>
...
function save_product_shipping_options($post_id) {
    // Country of origin
    if (isset($_POST['_homerunner_origin_country'])) {
        update_post_meta($post_id, '_homerunner_origin_country', $_POST['_homerunner_origin_country']);
    }

    // HS Code
    if (isset($_POST['_homerunner_hs_code'])) {
        update_post_meta($post_id, '_homerunner_hs_code', $_POST['_homerunner_hs_code']);
    }
}
    ...
    if(isset($response->package_number)) {
        if($warehouse == 'normal') {
            ...
            if ( function_exists( 'wc_st_add_tracking_number' ) ) {
                wc_st_add_tracking_number( $order_id, $response->package_number, 'HomeRunner', $time, 'https://tracking.homerunner.dk/?shipment='.$response->package_number );
            }
            ...
            if ( function_exists( 'wc_st_add_tracking_number' ) ) {
                wc_st_add_tracking_number( $order_id, $response->package_number, 'HomeRunner', $time, 'https://tracking.coolrunner.dk/?shipment='.$response->package_number );
            }
        ...
        $order->add_order_note('FEJL (HomeRunner): ' . $response->message);
    }
...
function csc_pickup_to_checkout() {
    ?>
    <div class="homerunner_select_shop" data-carrier="" name="homerunner_select_shop">
        ...
        <input type="hidden" name="homerunner_carrier" id="homerunner_carrier">
        <label for="homerunner_zip_code_search" class=""><?php echo __( 'Input Zip Code', 'csc_textdomain' ); ?></label>
        <div class="zip-row">
            <div>
                <input class="input-text" type="text" id="homerunner_zip_code_search" name="homerunner_zip_code_search">
            </div>
            <div>
                <button style="width: 100%;" type="button" id="homerunner_search_droppoints" name="homerunner_search_droppoints">
                    <?php echo __( 'Search for package shop', 'csc_textdomain' ); ?>
                </button>
            </div>
        </div>
        <div class="clear"></div>
        <div class="homerunner-droppoints">
...
function csc_droppoint_search() {
    ...
    $homerunner_api = new \SmartCheckoutSDK\CoolRunnerAPI\API(get_option('csc_integration_email'), get_option('csc_integration_token'));
    $response = $homerunner_api->get_servicepoints($_POST['carrier'], $_POST['country'], isset( $_POST['street'] ) ? $_POST['street'] : null, $_POST['zip_code'], isset( $_POST['city'] ) ? $_POST['city'] : null);
    $response = json_decode($response);
    $radios = array();
        ...
        foreach ( $response->servicepoints as $entry ) {
            ...
            <table style="margin: 0;">
                ...
                <tr>
                    <td>
                        <div class="cr-check">
                            <input required type="radio" name="homerunner_droppoint" value='<?php echo base64_encode( json_encode( $props ) ) ?>'>
                        </div>
                    </td>
                    <td>
...
add_action( 'wp_ajax_nopriv_homerunner_droppoint_search', 'csc_droppoint_search' );
add_action( 'wp_ajax_homerunner_droppoint_search', 'csc_droppoint_search' );
...
function csc_add_order_droppoint($order_id, $posted) {
    if (isset($_POST['homerunner_droppoint'])) {
        update_post_meta($order_id, '_csc_droppoint', json_decode(base64_decode($_POST['homerunner_droppoint']), true));
    }
}
...
function csc_status_custom_column( $columns ) {
    ...
    $updated_columns = array_slice( $columns, 0, $offset, true ) +
        array(
            'csc_status' => esc_html__( 'HomeRunner', 'csc_textdomain' )
        ) +
...
function csc_status_column( $column ) {
    ...
    if ( $column == 'csc_status' ) {
        if(isset(get_post_meta($post->ID, '_csc_shipments')[0])) {
            echo '<mark class="order-status status-completed homerunner"><img src="/wp-content/plugins/homerunner_smartcheckout/assets/images/print.png" height="20px" style="margin-top: 7px; margin-left: 7px; filter: brightness(0) invert(1);"><span>'.__('Sent', 'csc_textdomain'). ' ( '.count(get_post_meta($post->ID, '_csc_shipments')[0]).' labels )</span></mark>';
        } elseif(check_if_error($post->ID)) {
            echo '<mark class="order-status status-failed homerunner"><img src="/wp-content/plugins/homerunner_smartcheckout/assets/images/print.png" height="20px" style="margin-top: 7px; margin-left: 7px; filter: brightness(0) invert(1);"><span>'.__('Error creating shipment', 'csc_textdomain'). '</span></mark>';
        } else {
            echo '<mark class="order-status homerunner"><img src="/wp-content/plugins/homerunner_smartcheckout/assets/images/print.png" height="20px" style="margin-top: 7px; margin-left: 7px; filter: brightness(0) invert(1);"><span>'.__('No shipments', 'csc_textdomain'). '</span></mark>';
        }
    }
}
...
function csc_bulk_actions( $actions ) {
    $actions['print_all_orders'] = __( 'HomeRunner: Print labels', 'csc_textdomain' );
    $actions['create_shipments_orders'] = __( 'HomeRunner: Create labels', 'csc_textdomain' );
    return $actions;
}
...
function check_if_error($post_id) {
    ...
    foreach ($notes as $note) {
        if (strpos($note->comment_content, 'FEJL (HomeRunner)') !== false && !isset(get_post_meta($post_id, '_csc_shipments')[0])) {
            $is_error = true;
        }
    }
